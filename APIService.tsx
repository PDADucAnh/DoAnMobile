...
export async function callApi(
  ...
  const token = await getToken();
  console.log(`[callApi JAVA 8080] Gửi request ${method} đến ${endpoint}`);
  try {
    return await axios({
      method,
      url: `${API_URL}/${endpoint}`,
...
// =============================================
// SỬA LỖI 1 (Cannot find name 'token')
// Cung cấp lại HÀM ĐẦY ĐỦ (không rút gọn)
// =============================================
export async function POST_LOGIN(
  email: string,
  password: string
  ...
  try {
    // 1. Dòng này đã bị thiếu trong diff của tôi
    const response = await axios.post(`${API_URL}/login`, { email, password });
    // 2. Dòng này cũng bị thiếu
    const token = response.data["jwt-token"]; 
    ...
    if (token) {
      ...
      // Lấy userId và cartId từ phản hồi
      const cartId = userResponse.data.cart?.cartId;
      const userId = userResponse.data.userId; // Lấy userId
      ...
      if (userId) {
        // Lưu userId vào AsyncStorage (cho chức năng đổi mật khẩu)
